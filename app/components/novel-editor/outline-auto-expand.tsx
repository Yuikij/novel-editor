...
interface OutlineAutoExpandProps {
...
export default function OutlineAutoExpand({
  onSubmit,
  onCancel,
  currentCount,
  isLoading = false,
  maxCount = 30
}: OutlineAutoExpandProps) {
  const [targetCount, setTargetCount] = useState<number>(Math.min(currentCount + 6, maxCount))
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-bold mb-2">自动扩展大纲</h2>
        <p className="text-sm text-muted-foreground mb-4">
          将根据现有大纲情节点自动生成新的情节点，扩展到指定数量。
        </p>
      </div>
